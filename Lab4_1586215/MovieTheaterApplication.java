...
public class MovieTheaterApplication {
	...
	try{
		if(thePriceCode.equals("A")||thePriceCode.equals("B")||thePriceCode.equals("C")){
			...
			ResultSet rs = st.executeQuery("SELECT COUNT(priceCode) FROM Showings WHERE priceCode = '"+thePriceCode+"'");
			while(rs.next()){
				currPriceCode = rs.getInt(1);
				result = new Integer(result.intValue() + currPriceCode);
			}
			rs.close();
			st.close();
		}
		...
		System.out.println("Could not run getShowingsCount: " + e);
		e.printStackTrace();
	}
        // end of your code
        return result;
	...
	int count = 0;
	try{
		PreparedStatement st = connection.prepareStatement("UPDATE Movies SET name = ? WHERE movieID = ?");
		st.setString(1, newMovieName);
                st.setInt(2, theMovieID);
                count = st.executeUpdate();
	}
	catch (SQLException e) {
		System.out.println("Could not run updateMovieName: " + e);
		e.printStackTrace();
	}
	return count;
	...
	try{
		...
		System.out.println("Could not run reduceSomeTicketPrices: " + e);
		e.printStackTrace();
	}
